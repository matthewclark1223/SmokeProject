...
library(zoo)
library(formattable)
load("../ModelObjects/feefit.rda")
load("../ModelObjects/residuals.rda")
load("../ModelObjects/CVResidualMod.rds")
```
...
Visitation data was obtained through the [National Park Service Visitor Use Statistics Portal](https://irma.nps.gov/STATS/). These data are generally collected via car counters, permit information, and concessionaire reporting. Methods of collection vary from park to park. Unit specific information can be found on the National Park Service Visitor Use Statistics Portal. Although collection methods are inconsistent across the sample, all data was taken as reported. We retrieved monthly data for all national park units in the Intermountain and Pacific West regions for years 1980 through 2019. To reduce issues regarding park seasonality, we then subset our data to only the three high season months associated with each park. For example, only Summer (June, July, Aug) data was used for Yellowstone National Park.

### 2.1.2. Visitation Fees

Historic visitation fee data is not readily available from the National Park Service. The recreational visitor fees however, have been listed on the individual park websites historically. We accessed these historic webpages and aggregated the single visitor entry fee data using the [Internet Archive](https://archive.org/web/). We were able to access historic webpages for the years 1997-2018. Archived web pages were unavailable for years prior to 1997. For years 1980-1996, we performed trendline extension via linear regression to estimate the recreational visitor fee for each park, each year per the following:  

 
```{r include=FALSE, cache=TRUE}
FEdat<-read.csv("../Data/LauraStoddardEntryFeeCsv.csv")
...
$$
\begin{aligned}
y_{tp} &\sim \begin{cases}\operatorname{Pois}(\mu,\lambda)\\ \mu = B_{0}+ B_{t}*X_{t} +B_{p}*X_{p}
\end{cases}
\end{aligned}
$$

which yields the visitation estimate \(y\) for a given park \(_{p}\) in a given year \(_{t}\). Model details and validation can be found in Supplemental (S1).

To estimate the visitor revenue for each park, each year, we multiplied the observed or estimated park fee by the number of visitors recorded for each park. This is an oversimplification of the visitation fee system and likely overestimates the fees paid by recreational visitors as it does not account for multiple visitors who may share one vehicle. As a result of this discrepancy, we caution that backcasted income losses should be taken at an order of magnitude level only, not as specific dollar estimates
...
#predicted vis fees 
FEdat<-read.csv("../Data/ParkFees.csv")
...
knitr::include_graphics(path.expand("../Train_Test.png"))
```
...
load("../ModelObjects/CVResidualMod2.rds")
...
load("../ModelObjects/CVResidualMod3.rds")
...
load("../ModelObjects/CVResidualMod4.rds")
...
load("../ModelObjects/CVResidualMod5.rds")
...
load("../ModelObjects/CVResidualMod6.rds")
...
load("../ModelObjects/CVResidualMod7.rds")
