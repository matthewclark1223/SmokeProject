...
p2<-dat%>%filter(UnitCode == "GLAC")%>%group_by(Year)%>%
   ...
   scale_y_continuous(name="Smoke (PM 2.5)",expand = c(0, 0), limits = c(0, 1.25e-9))+mytheme
...
p2<-dat%>%filter(UnitCode == "OLYM")%>%group_by(Year)%>%
    ...
    guides(alpha = FALSE) + 
    theme_classic()+  scale_x_continuous(name="Date",breaks=c(1980,1990,2000,2010,2018))+
   scale_y_continuous(name="Smoke (PM 2.5)",expand = c(0, 0), limits = c(0, 1.0e-9))+mytheme
