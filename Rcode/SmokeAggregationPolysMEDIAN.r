...
for (i in 1:length (list_all)) {
  ...
  out_median<-as.data.frame(rowMedians(as.matrix(out)))  #take median of the assign list above
  ...
  colnames(out_median)[1] <- n #give values column name based on month
  name<-paste0("/Users/Matthewclark989/Documents/output_medians/",n) #create empty folder to save to
  newname<-paste0(name, ".csv")
  write.csv(out_median, newname, row.names = FALSE) #save as csv
  
}

#loads those .csv and makes into single df
list_dates = list.files(path="/Users/Matthewclark989/Documents/output_medians/", pattern="*.csv", full.names=TRUE) #same folder you saved above output

final_medians<-list_dates %>%
  map(read_csv) %>%
  reduce(cbind)

#append park code
final_medians<-data.frame(park=park_codes, final_medians)

#save
write.csv(final_medians, "/Users/Matthewclark989/Documents/final_medians.csv", row.names = FALSE)
